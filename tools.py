...
def _statement_value(statement: Any, labels: tuple[str, ...], column: int = 0) -> float:
    """Read a statement line without assuming every issuer reports every field."""
...
def build_dossier(ticker: str = "NVDA") -> dict[str, Any]:
    """Build a live dossier for any Yahoo Finance-supported ticker symbol."""
    ...
    assets = [_billions(_statement_value(balance_sheet, ("Total Assets",), index)) for index in (0, 1)]
    liabilities = [_billions(_statement_value(balance_sheet, ("Total Liabilities Net Minority Interest", "Total Liabilities"), index)) for index in (0, 1)]
    ...
    revenue = [_billions(_statement_value(income_statement, ("Total Revenue",), index)) for index in (0, 1)]
    operating_income = [_billions(_statement_value(income_statement, ("Operating Income",), index)) for index in (0, 1)]
